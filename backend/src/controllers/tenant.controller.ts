...
export const createTenant = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  logger.info('Creating tenant [DEBUG-V2]', { roomId, fullName, lineUserId });
  ...
  if (userId) {
    ...
    if (existingUser) {
      ...
      logger.info(`Linking existing user ${email} to new tenant`);
    } else {
      ...
      logger.info(`Auto-created user account for tenant: ${email}`);
    }
  }
  ...
  logger.info(`Tenant created: ${tenant.fullName}`, {
    tenantId: tenant.id,
    roomId,
...
export const updateTenant = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  logger.info('Updating tenant [DEBUG-V2]', { id, fields: Object.keys(updateData) });
  ...
  if (updateData.hasOwnProperty('lineUserId')) {
    if (updateData.lineUserId) {
      ...
      logger.info('Disconnecting lineUser for tenant', { tenantId: id });
  ...
  logger.info(`Tenant updated: ${tenant.fullName}`, {
    tenantId: id,
...
export const deleteTenant = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (activeTenants === 0) {
    await prisma.room.update({
      where: { id: existingTenant.roomId as string },
      data: { status: 'available' },
    });
  }

  logger.info(`Tenant deleted: ${existingTenant.fullName}`, {
    tenantId: id,
 ...
 * @param res - Express response
 */
export const linkLineUser = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  ...
  logger.info(`LINE user linked to tenant: ${tenant.fullName}`, {
    tenantId: id,
    lineUserId,
 ...
 * @param res - Express response
 */
export const resetTenantPassword = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;

  logger.info(`Password reset requested for tenant: ${id}`, {
  ...
  if (!tenant) {
    logger.warn(`Tenant not found for password reset: ${id}`);
    res.status(404).json({
      success: false,
      message: 'Tenant not found',
    });
  ...
  if (!tenant.user) {
    logger.warn(`User account not found for tenant: ${id}`);
    res.status(404).json({
      success: false,
  ...
  logger.info(`Password reset successful for tenant: ${tenant.fullName}`, {
    tenantId: id,
  ...
  if (tenant.lineUser) {
    try {
      ...
      logger.info(`LINE notification sent for password reset: ${tenant.fullName}`);
    } catch (error) {
      logger.error('Failed to send LINE notification for password reset', { error });
