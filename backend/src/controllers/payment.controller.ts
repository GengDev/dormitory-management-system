...
import { PrismaClient, PaymentMethod, BillStatus, NotificationType } from '@prisma/client';
...
export const getPayments = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (req.user?.role === 'tenant') {
    ...
    if (month || year) {
      const filterMonth = month ? parseInt(month as string, 10) : null;
  ...
  const transformedPayments = payments.map((payment: any) => {
    if (!payment.bill) return payment;
...
export const createPayment = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  logger.info(`Payment recorded: ${result.payment.id}`, {
...
export const approvePayment = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    logger.error('Failed to queue payment approval notification:', error);
  }
...
export const rejectPayment = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    logger.error('Failed to queue payment rejection notification:', error);
  }
 ...
 */
export const submitTenantPayment = asyncHandler(async (req: AuthRequest, res: Response) => {
  const {
    billId,
    amount,
    paymentMethod,
  ...
  logger.info(`Tenant submitted payment for bill ${bill.billNumber}`, {
    tenantId: tenant.id,
    billId,
    paymentId: result.payment.id,
  });
