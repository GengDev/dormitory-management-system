...
export function initializeSocketIO(io: SocketIOServer): void {
  ...
  publicChat.on('connection', (socket: Socket) => {
    logger.info('Public chat user connected', { socketId: socket.id });

    socket.on('join_private_chat', async (data: { name: string; roomId: string; isAdmin?: boolean }) => {
      ...
      try {
        if (!isAdmin) {
          ...
          if (!chatRoom) {
            ...
            logger.info(`Created new public chat room: ${chatRoom.id} (Session: ${roomId})`);
          } else {
            logger.info(`Joined existing public chat room: ${chatRoom.id} (Session: ${roomId})`);
          }
        ...
        logger.error('Error creating/joining public chat room', { error });
      }

      logger.info(`Private chat user joined: ${name} (Room: ${roomId}, Admin: ${isAdmin})`, { socketId: socket.id });
    ...
    socket.on('join_admin_room', (data: { name: string; isAdmin?: boolean }) => {
      const { name, isAdmin = false } = data;
      socket.data.name = name;
      socket.data.isAdmin = isAdmin;
      socket.join('admin_room');
      logger.info(`Admin joined admin room: ${name}`, { socketId: socket.id });
    });

    socket.on('send_message', async (data: { message: string; isAdmin?: boolean; roomId?: string }) => {
      ...
      try {
        ...
        if (chatRoom) {
          ...
          logger.info(`Private chat message persisted in room ${chatRoom.id} (Sender: ${senderIsAdmin ? 'Admin' : 'Guest'})`);

        } else {
          logger.warn(`Chat room not found for id/session: ${incomingRoomId}, message not persisted.`);
        }
      } catch (error) {
    ...
    socket.on('disconnect', () => {
      logger.info('Public chat user disconnected', { socketId: socket.id });
    });
  });
  ...
  io.on('connection', (socket: Socket) => {
    ...
    logger.info(`Socket connected: ${user.email}`, { userId: user.id });
    ...
    socket.on('join_room', async (chatRoomId: string) => {
      try {
        ...
        logger.info(`User joined chat room: ${chatRoomId}`, { userId: user.id });
    ...
    socket.on('leave_room', (chatRoomId: string) => {
      ...
      logger.info(`User left chat room: ${chatRoomId}`, { userId: user.id });
    });
    ...
    socket.on('send_message', async (data: {
      ...
      try {
        ...
        logger.info(`Message sent in chat room: ${chatRoomId}`, {
     ...
     */
    socket.on('mark_read', async (data: { chatRoomId: string; messageId: string }) => {
      try {
     ...
     */
    socket.on('disconnect', () => {
      logger.info(`Socket disconnected: ${user.email}`, { userId: user.id });
    });
  });
...
async function checkChatRoomAccess(
  ...
  try {
    ...
    return (
      chatRoom.tenantId !== null ||
      chatRoom.guestUserId === userId ||
      chatRoom.adminUserId === userId
    );
  } catch (error) {
    logger.error('Error checking chat room access', { error });
    return false;
  }
}
