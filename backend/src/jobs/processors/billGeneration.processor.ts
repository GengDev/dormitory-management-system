 ...
 */
export async function processBillGenerationJob(data: {
  tenantId: string;
  billingMonth: string;
  dueDate: string;
  ...
  try {
    ...
    if (!tenant || tenant.status !== 'active' || tenant.deletedAt) {
      logger.warn(`Cannot generate bill: tenant ${tenantId} not found or inactive`);
      return { success: false };
    }

    if (!tenant.roomId || !tenant.room) {
      logger.warn(`Cannot generate bill: tenant ${tenantId} is not assigned to a room`);
      return { success: false };
    }
    ...
    if (existingBill) {
      logger.info(`Bill already exists for tenant ${tenantId} in ${billingMonth}`);
    ...
    logger.error('Failed to process bill generation job', {
      error: error.message,
      data,
    });
    throw error;
